...
bool Map::addObstacle(float stateX, float stateY, float stateDelta, MotType motion){
  ...
  obstacles.polygons[idx].points[ci].setXY(center_x + cos(scalePI( deg2rad(0)   + circle_rot ) ) * r, center_y + sin(scalePI( deg2rad(0)   + circle_rot ) ) * r);
  ci += 1;
...
bool Map::nextPoint(bool sim,float stateX, float stateY, bool nextmowpoint){
  ...
  if (wayMode == WAY_DOCK){
    ...
    // check if curmowerpos is inside obstacle - this will prevent us from finding a path - use 0.05 to be 0.01 smaller than findPath
    while (true) {
      int ob_idx = isPointInsideObstacle(curmowerpos, -1, -0.03);
...
bool Map::nextDockPoint(bool sim, bool nextpoint){    
  ...
  if (shouldDock){
    ...
    if (dockPointsIdx > 0){
      ...
      if (!sim) {
        trackReverse = (dockPointsIdx >= dockPoints.numPoints-4) ; // undock reverse only in dock
      }              
      if (!sim) trackSlow = true;      
      return true;
    } else {
