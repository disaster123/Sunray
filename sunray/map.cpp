...
bool Map::addObstacle(float stateX, float stateY, float stateDelta, MotType motion){
  ...
  float center_x = stateX + cos( scalePI( scalePI(stateDelta) + scalePI(deg2rad(move_angle)) ) ) * (r*2/3);
  float center_y = stateY + sin( scalePI( scalePI(stateDelta) + scalePI(deg2rad(move_angle)) ) ) * (r*2/3);
