...
int get_turn_direction_preference() {
  ...
  float r = MOWER_GPS_TO_FRONT_DIAMETER;
...
void trackLine(bool runControl){  
  ...
  if (SMOOTH_CURVES)
    ...
    targetReached = (targetDist_moved < 0.05) || (targetDist < 0.03);
  ...
  if ( (targetDist_moved < 1.2) || (lastTargetDist < 1.2) || (fabs(distToPath) > 0.7) ||
       rotateLeft || rotateRight ) {
    if (SMOOTH_CURVES)
      angleToTargetFits = (fabs(trackerDiffDelta)/PI*180.0 < 120);
    else     
      angleToTargetFits = (fabs(trackerDiffDelta)/PI*180.0 < 15);
  } else {
  ...
  if ( maps.isUndocking() && gps.solution != SOL_FIXED ) {
    ...
    if (millis() > lastFixTime && dist > 1 && dist < 3) {
      activeOp->onGpsFixTimeout();
    }
  }
