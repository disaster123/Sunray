...
void trackLine(bool runControl){  
  ...
  float linear = 1.0;  
  bool mow = true;
  if ( maps.isUndocking() || maps.isDocking() || stateOp == OP_DOCK ) mow = false;
  float angular = 0;      
  float targetDelta = pointsAngle(stateX, stateY, target.x(), target.y());      
  ...
  if (!angleToTargetFits){
    ...
    if (!targetReached && targetDist < (MOWER_SIZE*1.5/100.0) && !maps.isDocking() && !maps.isUndocking()) {
      ...
      if ((maps.checkpoint( stateX_infrontof, stateY_infrontof, -0.04, false )) && 
  ...
  if (targetReached){
    ...
    bool straight = maps.nextPointIsStraight();
    int curmowpoint = maps.mowPointsIdx;
    if (!maps.nextPoint(false,stateX,stateY,true)){
      if (curmowpoint > maps.mowPointsIdx && maps.mowPointsIdx == 0 && maps.wayMode == WAY_MOW) {
        // finish
