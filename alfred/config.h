...
#define MOW_OVERLOAD_CURRENT 2.3    // mowing motor overload current (amps)
...
//#define DEBUG_HTTP_SERVER true       // debug msg for http server
...
#define BUMPER_DEADTIME 0  		// linear motion dead-time (ms) after bumper is allowed to trigger
...
#define INVALID_GPS_TIMEOUT 30 // time in seconds to navigate with invalid GPS
#define IGNORE_GPS_FIX_AFTER_INVALID 3.0 // time in seconds to ignore GPS fix after going into invalid solution
...
#define KIDNAP_DETECT_DISTANCE_DOCK_UNDOCK 1  // distance from dock in (m) to use KIDNAP_DETECT_ALLOWED_PATH_TOLERANCE_DOCK_UNDOCK
