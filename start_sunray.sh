...
( sleep 30; sudo bash -c 'rmmod 8821cu && ( insmod /home/pi/Sunray/8821cu.ko rtw_drv_log_level=1 rtw_led_ctrl=1 rtw_vht_enable=1 rtw_power_mgnt=0 rtw_ips_mode=0; systemctl restart networking; systemctl restart wpa_supplicant;  )' >~/Sunray/kernel_driver.log 2>&1 ) &
...
echo "----BLE config----"
echo 6 > /sys/kernel/debug/bluetooth/hci0/conn_min_interval  # 6
echo 6 > /sys/kernel/debug/bluetooth/hci0/conn_max_interval  # 6
echo 1 > /sys/kernel/debug/bluetooth/hci0/conn_latency  # 1
